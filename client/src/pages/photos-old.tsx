...
export default function PhotosPage() {
  ...
  // Basitleştirilmiş state management
  ...
  const startSessionMutation = useMutation({
    mutationFn: async (data: { tcNumber: string; selectedModelIds: string[] }) => {
      const response = await apiRequest('/api/photo-matching/start', {
        method: 'POST',
        body: JSON.stringify(data),
      });
      return response as any; // API response type
    },
    onSuccess: (data: any) => {
      setCurrentSession({
        sessionId: data.sessionId || data.id,
        status: 'face_detection' as const,
        progress: 0,
  ...
  // Download results - gerçek endpoint kullan
  const downloadMutation = useMutation({
    mutationFn: async (tcNumber: string) => {
      ...
      // Response blob olarak al (ZIP dosyası)
    ...
    onSuccess: (blob, tcNumber) => {
      ...
      toast({
        ...
        description: "Yüz eşleştirme sonuçları indirildi",
      });
    },
    onError: (error: any) => {
      ...
      if (error.message?.includes('embedding')) {
        userMessage = "⚠️ Lütfen önce fotoğraf yükleyip yüz tespiti yapın";
      } else if (error.message?.includes('session')) {
        userMessage = "⚠️ Oturum bulunamadı. Lütfen işlemi yeniden başlatın";
                      ...
                      <CardContent className="p-4">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <Button
                              size="sm"
                              onClick={() => downloadMutation.mutate(tcNumber)}
